...
/**
 * ⚡ Performance: SvgEdge is memoized to prevent re-renders when other nodes move.
 * We use flattened primitives for coordinates to ensure shallow comparison works perfectly.
 */
...
/**
 * ⚡ Performance: ArchitectNode is memoized. Since FunnelProvider preserves
 * object identity for non-moving nodes, this skips expensive re-renders
 * for everything except the currently dragged node.
 */
...
export default function FunnelArchitect() {
  ...
  /**
   * ⚡ Performance: Map nodes by ID to allow O(1) lookup during edge coordinate calculation.
   * This reduces total complexity from O(N*E) to O(N+E).
   */
